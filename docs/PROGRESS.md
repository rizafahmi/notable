...
## Current State
- The display typeface now actually renders. `priv/static/fonts/notable-display.woff2` had been Fraunces' **Vietnamese** subset since `afab09e` (2026-05-03) - 111 codepoints with no `N`, `o`, `t`, `b`, `l` or `e` - so the wordmark, every `font-display` heading, and the `/cloud` word cloud silently fell back. It is now built from upstream Fraunces v1.003 (OFL) by [`scripts/fonts/build-display-font.sh`](../scripts/fonts/build-display-font.sh) as a weight-only `opsz=72` display cut over `latin` + `latin-ext`, and `--font-display` carries a real fallback stack. `test/notable_web/display_font_test.exs` reads the shipped binary and fails if the glyphs or the fallback go missing again. See [Milestone 17 log](milestones/17-display-font-fix/milestone-log.md)
- **Offline submission design is written and piece 1 of 4 is built.** The captain watched the audience pages fail on venue wifi (2026-08-23). The decided design — plain HTTP submission endpoints, a client outbox, a service worker shell cache, and an honest offline banner; feedback and questions only, never votes or tips — is in [docs/superpowers/specs/2026-08-25-offline-submission-design.md](superpowers/specs/2026-08-25-offline-submission-design.md). The **service worker shell cache** is live in code: `/` and `/questions` render offline once visited, digested assets are cache-first from the `phx.digest` manifest, `/admin` and `/live` are never touched, the cache is stamped per build and old caches are deleted on activate, and `NOTABLE_SERVICE_WORKER=off` is the kill switch ([OPERATIONS.md → Service Worker](OPERATIONS.md#service-worker)). Verified offline, against a hung server (3 s fallback), across two builds, and through the kill switch — see [Milestone 18 log](milestones/18-offline-shell-cache/milestone-log.md). Remaining pieces are queued as `notable-offline-http-endpoints`, `notable-offline-outbox`, `notable-offline-banner`
- **Deploy runner is pinned to `ubuntu-22.04` and must not be changed to `ubuntu-latest`.** The first live dispatch (run `32682797726`, 2026-08-24) built cleanly and then died on the VM with `` `GLIBC_2.38' not found ``: `mix release` bundles ERTS linked against the build machine's glibc, glibc is forward-compatible only, and `ubuntu-latest` had rolled to Ubuntu 24.04 (glibc 2.39) while the VM runs **Debian 12, glibc 2.36, x86_64**. The safety ordering held - migrations run before the symlink swap, so `current` was untouched and the bad release was removed. The drift guard had asserted `runs-on: ubuntu-latest`, so the suite *required* the broken config; it now asserts the invariant instead (pinned, never `-latest`, runner glibc `<=` target glibc, unknown images rejected). The pin expires when GitHub retires the image and will need revisiting. See [OPERATIONS.md → Build Runner And The Target's glibc](OPERATIONS.md#build-runner-and-the-targets-glibc) and [ADR-026](decisions/ADR-026-pin-the-deploy-runner-to-the-target-glibc.md)
- Deployment is automated from GitHub Actions for [#37](https://github.com/rizafahmi/notable/issues/37): `workflow_dispatch`-only `Deploy` and `Rollback` workflows build the release on the runner, ship it over SSH, migrate before swapping `current`, restart via systemd, and prune with bounded retention while keeping the SQLite file and WAL companions unreachable. Secrets exist and the path reaches the VM; the glibc fix above has **not** been confirmed by a live dispatch. See [OPERATIONS.md](OPERATIONS.md#deployment), [Milestone 16 log](milestones/16-deployment-automation/milestone-log.md), and [ADR-025](decisions/ADR-025-build-releases-in-github-actions.md)
- `/cloud` and `/cloud-overlay` now render a **true packed word cloud** instead of a centred `flex-wrap` list: `Notable.WordCloud.Style` derives each word's colour, within-level size variation and rotation from a salted FNV-1a hash of the word (so equal counts no longer render in one colour at one size), then forces contrast with the two words that first appeared before it — never the same colour family, never two verticals in a row, never three horizontals — so even a two-word cloud shows one of each orientation in two colours; that history is append-only, so nothing restyles mid-talk. The `WordCloud` hook in `assets/js/app.js` does spiral placement and nothing else: a word never moves once placed, and a word that grows is re-placed within 260px of where it sat. The two extra cloud colours are design tokens now (`--color-accent-amber`, `--color-accent-rose`, display-only — see [DESIGN.md](DESIGN.md)). See [Milestone 17 log](milestones/17-packed-word-cloud/milestone-log.md)
...
- Release status: mobile donor and OBS-sized browser smoke checks passed; deploy automation has reached the VM once and failed on the glibc mismatch above. Next: the captain dispatches **Actions → Deploy** again with the pinned runner, then the live Mayar transaction smoke check
- Known issue (not addressed here): the captain's site is being served by a manual process. The runner pin makes a working deploy possible; it is not itself a fix for that

## Completed
- [x] Display font: replace the Vietnamese-only Fraunces subset with a Latin display cut and give `--font-display` a fallback stack - see [Milestone 17 log](milestones/17-display-font-fix/milestone-log.md).
- [x] Offline submission spec + service worker shell cache for `/` and `/questions` — see [Milestone 18 log](milestones/18-offline-shell-cache/milestone-log.md).
...
- [x] `/cloud` packed word-cloud layout: colour/size-variation/rotation decided (and unit-tested) in Elixir, geometry-only JS hook, stable across live updates — see [Milestone 17 log](milestones/17-packed-word-cloud/milestone-log.md).
...
## Next Steps
1. Offline submission, remaining pieces in order, each against the [spec](superpowers/specs/2026-08-25-offline-submission-design.md): `notable-offline-http-endpoints` (POST `/feedback` and `/questions` with `client_id` idempotency, bounded `submitted_at`, CSRF kept, cooldown applied), then `notable-offline-outbox` (IndexedDB queue, "Tersimpan"/"Terkirim" in the client, token refresh on 403), then `notable-offline-banner` (honest offline status, replaces the red reconnect toasts on the cached form).
...
- Milestone 17 log: [docs/milestones/17-packed-word-cloud/milestone-log.md](milestones/17-packed-word-cloud/milestone-log.md)
...
- Milestone 18 log: [docs/milestones/18-offline-shell-cache/milestone-log.md](milestones/18-offline-shell-cache/milestone-log.md)
