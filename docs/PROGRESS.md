...
- `/cloud` and `/cloud-overlay` now render a **true packed word cloud** instead of a centred `flex-wrap` list: `Notable.WordCloud.Style` derives each word's colour, within-level size variation and rotation from a salted FNV-1a hash of the word itself (so equal counts no longer render in one colour at one size), and the `WordCloud` hook in `assets/js/app.js` does spiral placement and nothing else. Placement is stable — a word never moves once placed — and the pre-hook/no-JavaScript state is still a readable wrapping list. See [Milestone 17 log](milestones/17-packed-word-cloud/milestone-log.md)
