...
- The deploy workflow's `runs-on` is a correctness constraint, not a preference: `mix release` bundles ERTS linked against the runner's glibc, glibc is forward-compatible only, and the VM is older than `ubuntu-latest`. Never set it to a floating `-latest` label. The rule, the target's actual glibc, and what to do when the pinned image is retired are in [OPERATIONS.md](docs/OPERATIONS.md#build-runner-and-the-targets-glibc) and [ADR-026](docs/decisions/ADR-026-pin-the-deploy-runner-to-the-target-glibc.md); `test/notable/deploy/deploy_workflow_test.exs` enforces it.
