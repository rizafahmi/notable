...
- The `/cloud` packed layout splits cleanly and must stay split: `Notable.WordCloud.Style` decides a word's colour, its size variation within its level and whether it is rotated — hashed from the word, with tone and rotation forced to contrast with the few words that first appeared before it that day (an append-only history, so nothing restyles mid-talk), all unit-tested — while the `WordCloud` hook in `assets/js/app.js` decides *only* where each word sits. There is no JavaScript test runner here and none is to be added, so a decision that can be tested belongs in Elixir. Two hook mechanics are load-bearing, not stylistic: positions go into a `<style>` element in `<head>` keyed by `[data-word]`, and `.cloud-packed` goes on `<html>` — both because a LiveView patch strips attributes the server template does not own, and the absent class doubles as the no-JavaScript fallback. See [Milestone 17 log](docs/milestones/17-packed-word-cloud/milestone-log.md).
